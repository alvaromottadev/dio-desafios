...
public class CardService {
    ...
    public void moveToNextColumn(final Long cardId, final List<BoardColumnInfoDTO> boardColumnsInfo) throws SQLException{
        try{
            var dao = new CardDAO(connection);
            var currentColumn = validateCardState(dao, cardId, boardColumnsInfo, false);
    ...
    public void block(final Long id, final String reason, final List<BoardColumnInfoDTO> boardColumnsInfo) throws SQLException {
        try{
            ...
            var dto = optionalOrElseThrow(optional);
            isNotBlocked(dto, id);
    ...
    private void isBlocked(CardDetailsDTO dto, Long id) {
        if (!dto.blocked()){
    ...
    private void isNotBlocked(CardDetailsDTO dto, Long id) {
        if (dto.blocked()){
    ...
    private BoardColumnInfoDTO validateCardState(CardDAO dao, final Long cardId,
                                                 final List<BoardColumnInfoDTO> boardColumnsInfo,
                                                 final boolean checkFinalState) throws SQLException {
        var optional = dao.findById(cardId);
        var dto = optionalOrElseThrow(optional);
        isNotBlocked(dto, cardId);
        var currentColumn = boardColumnsInfo.stream()
                .filter(bc -> bc.id().equals(dto.columnId()))
                .findFirst()
                .orElseThrow(() -> new IllegalStateException("O card informado pertence a outro board"));
        if (checkFinalState && currentColumn.kind().equals(FINAL)) {
            throw new CardFinishedException("O card já foi finalizado");
        }
        return currentColumn;
    }

    private CardDetailsDTO optionalOrElseThrow(Optional<CardDetailsDTO> optional){
        return optional.orElseThrow(() -> new EntityNotFoundException("O card não foi encontrado"));
    }
